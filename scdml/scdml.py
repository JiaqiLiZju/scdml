...
def scdml(adata, obs_label="Celltype",
    ...
    record_keeper, _, _ = logging_presets.get_record_keeper("scdml_logs", "scdml_tensorboard")
    hooks = logging_presets.get_hook_container(record_keeper)
    dataset_dict = {"train": train_dataset, "val": val_dataset}
    model_folder = "scdml_saved_models"
...
def scdml_clf(adata, obs_label="Celltype",
    ...
    # get data and format
    # as type np.long and torch.long for classification
    data, labels = adata.X, adata.obs[obs_label].cat.codes.values.astype(np.long)
    ...
    classifier = MLP_clf(in_sz=out_sz, out_sz=clf_output_size, emb_szs=[out_sz], ps=0)
    ...
    record_keeper, _, _ = logging_presets.get_record_keeper("scdml_clf_logs", "scdml_clf_tensorboard")
    hooks = logging_presets.get_hook_container(record_keeper)
    dataset_dict = {"train": train_dataset, "val": val_dataset}
    model_folder = "scdml_clf_saved_models"
